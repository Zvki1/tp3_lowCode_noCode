(() => {
  ...
  const clearSelectionStyles = () => {
    document
      ...
      .forEach((t) => t.classList.remove("active", "invalid"));
    document
  ...
  const animateMove = (el, toStackEl) =>
    new Promise((resolve) => {
  ...
  const redrawStacks = () => {
    // preserve elements for smoother anims
    towers.forEach((stackEl, i) => {
  ...
  const trySelectOrMove = (towerIndex) => {
    ...
    if (selectedFrom === null) {
      if (!topOf(towerIndex)) {
        setMessage("Aucun anneau à déplacer ici", "warn");
        return;
      }
  ...
  const checkWin = () => {
    if (stacks[2].length === n || stacks[1].length === n) {
      setMessage(
        `Bravo ! Résolu en ${moves} coups (optimal: ${optimalMovesFor(n)})`,
        "win"
      );
    }
  };
  ...
  async function runDemo() {
    ...
    cancelDemo = false;
    runningDemo = true;
    setMessage("Démo en cours… Cliquez à nouveau pour arrêter.");
